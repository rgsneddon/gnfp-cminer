...
assert.match(src, /#define VERSION "1\.1\.3"/);
assert.doesNotMatch(src, /#define VERSION "1\.1\.2"/);
...
const pool = shouldAdmitMiner({ client: 'GNFPHash', version: '1.1.3' });
assert.equal(pool.ok, true, JSON.stringify(pool));
assert.equal(nodeAdmit({ client: 'GNFPHash', version: '1.1.3' }).ok, true);
...
assert.equal(shouldAdmitMiner({ client: 'gnfp-mine', version: '1.1.3' }).ok, false);
...
console.log('admit ok client=GNFPHash version=1.1.3', pool);
