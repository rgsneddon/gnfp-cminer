...
assert.match(src, /#define VERSION "1\.1\.1"/);
assert.doesNotMatch(src, /#define VERSION "1\.1\.0"/);
...
const pool = shouldAdmitMiner({ client: 'GNFPHash', version: '1.1.1' });
assert.equal(pool.ok, true, JSON.stringify(pool));
assert.equal(nodeAdmit({ client: 'GNFPHash', version: '1.1.1' }).ok, true);
assert.equal(shouldAdmitMiner({ client: 'GNFPHash', version: '1.1.0' }).ok, true);
...
assert.equal(shouldAdmitMiner({ client: 'gnfp-mine', version: '1.1.1' }).ok, false);
...
console.log('admit ok client=GNFPHash version=1.1.1', pool);
