...
assert.match(src, /#define VERSION "0\.5"/);
...
assert.match(src, /g_fee_offset/);
assert.match(src, /ensure_fee_conn/);
assert.match(src, /g_meets % FEE_EVERY\) == \(uint64_t\)g_fee_offset/);
assert.match(src, /Open that socket on the first fee share/);
assert.doesNotMatch(src, /conn_open\(&feec, g_host, g_port, g_tls, 1\) == 0/);

const pool = shouldAdmitMiner({ client: 'GNFPHash', version: '0.5' });
assert.equal(pool.ok, true, JSON.stringify(pool));
assert.equal(nodeAdmit({ client: 'GNFPHash', version: '0.5' }).ok, true);
assert.equal(shouldAdmitMiner({ client: 'GNFPHash', version: '0.4' }).ok, true);
...
assert.equal(shouldAdmitMiner({ client: 'gnfp-mine', version: '0.5' }).ok, false);
...
console.log('admit ok client=GNFPHash version=0.5', pool);
