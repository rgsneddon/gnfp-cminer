...
try {
  ...
  pool = shouldAdmitMiner({ client: 'GNFPHash', version: '1.1.5' });
  assert.equal(pool.ok, true, JSON.stringify(pool));
  assert.equal(nodeAdmit({ client: 'GNFPHash', version: '1.1.5' }).ok, true);
  ...
  assert.equal(shouldAdmitMiner({ client: 'gnfp-mine', version: '1.1.5' }).ok, false);
...
console.log('admit ok client=GNFPHash version=1.1.5', pool);
